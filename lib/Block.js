...
function createBlock({ node, after, parent, parentBlock, parentScope }) {
  ...
  const localBlocks = parentScope ? parentScope.$$.Block._blocks : blocks;
  const localMixins = parentScope ? parentScope.$$.Block._mixins : mixins;
  let constructor = node && localBlocks[node.name];
  let dBlockMatch;
...
function createMixin({ Mixin, value, match, elem, parentBlock, parentScope }) {
  new Mixin({
    value,
    match,
    elem,
