...
class Block {
        ...
        value: {
          ...
          evaluate: (expression, onChange, instance, forDElements, forDItem, forDEach) => {
            ...
            const func = new Function(
              '',
              'with(document.DwayneStore){try{$$.expr=eval("$$.expr = " + $$.expr);return $$.expr;}catch(e){}}'
            );
  ...
  watchArgs(arg, watcher) {
    ...
    if (!this.$$.args[arg]) {
      constructPublicScope(this.args, {
        [arg]: this.args[arg]
      }, this.$$.args);
    }
   ...
   */
  watchGlobal(global, watcher) {
    validate([global, watcher], ['string', 'function'], 'Block#watchArgs');

    if (!this.$$.global[global]) {
      constructPublicScope(this.global, {
...
function createBlock({ node, after, parent, parentBlock, parentScope }) {
  ...
  let dBlockMatch;
  let constructor = node && node.name ? blocks[node.name] : null;
  ...
  if (name === '#d-item') {
    ...
    constructPublicScope($$.scope = Object.create(parentScope), scopeValues, $$.privateScope);
  }
...
function wrap(Block, middlewares = []) {
  return new Arr(middlewares).reduce((Block, middleware) => {
    if (!isFunction(middleware)) {
      return Block;
    }

    const block = middleware(Block);

    return isInstanceOfBlock(block)
      ? block
      : Block;
  }, Block);
}

export { Block, Mixin, initApp, wrap };
