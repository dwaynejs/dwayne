  ...
  static init() {
    ...
    states
      .forEach((state) => {
        ...
        const _parent = states.find(({ stateName }) => stateName === parent);

        if (!_parent) {
          throw new Error(`No such parent state (${ parent }) found for the state (${ stateName })! (at Router.init)`);
        }
...
function getListeners(state, type) {
  ...
  return tree
    .object((listeners, {
      ...
      if (new Super(proto).hasOwn(listenerName) && state.name === stateName) {
        listeners.push(
...
export function registerState(state) {
  ...
  if (states.find(({ stateName: n }) => n === stateName)) {
