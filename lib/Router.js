...
function constructState(cls, { params, query, templateParams, hash }) {
  const state = new cls();
...
function initialize() {
  if (loaded && initialized && !routerLoaded) {
    ...
    if (router) {
      ...
      // TODO: support syntax like '/path?status(^all$)' and '/path?status(^all$)&'
      // TODO: set params, query, templateParams to objects
...
export function registerState(name, state) {
  validate([name], ['string'], 'registerState');
  ...
  proto.params = isObject(params) ? params : {};
  proto.query = isObject(query) ? query : {};
  proto.templateParams = isObject(templateParams) ? templateParams : {};
