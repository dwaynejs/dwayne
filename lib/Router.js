...
import HtmlElement, { find } from './HtmlElement';
...
window.on({
  ...
  DOMNodeInserted({ target }) {
    const anchor = new HtmlElement(target);

    if (anchor.name === 'a') {
      const { domcData } = target;
  
      if (!domcData.clickHandler) {
        domcData.clickHandler = anchor.on('click', (e) => {
          if (!anchor.attr('download')) {
            e.preventDefault();
...
function leave(state, title, url, push) {
  ...
  return dispatchEvent(event).then(() => {
    return beforeLoad(state, title, url, push);
  });
}
function load() {
  ...
  const { states } = Router.$;
  const maxMatch = new Super(states).max(({ urlRegExp }) => {
    const match = current.match(urlRegExp);

    return match && match[0].length;
  });
  const minLevel = new Super(states)
    .filter(({ urlRegExp }) => {
      const match = current.match(urlRegExp);
