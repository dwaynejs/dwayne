...
import { isNil, isRegExp, isString } from './helpers';
...
function makeRoute(options) {
  ...
  let unsubscribe;
  let beforeLeaveCalled;

  return (Block) => (
    class extends Block {
      static template = `<d-if if="{__isCurrentRoute__}">${ Block.template }</d-if>`;
  ...
  function callBeforeLeave(route) {
    if (route.beforeLeaveRoute && !beforeLeaveCalled) {
