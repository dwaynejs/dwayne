...
import { isFunction, assign, validate, toArray, supportSymbol } from './helpers';
			...
			if (++proxy.$$.called < proxy.$$.canBeCalled) {
				...
				if (sync) {
					for (let i = 0; i < before.length; i++) {
						args = before[i].call(context, toArray(args), proxy);
					}

					ret = func.apply(context, toArray(args));

					for (let i = 0; i < after.length; i++) {
						ret = after[i].call(context, ret, proxy);
					}
...
if (supportSymbol) {
  Func.prototype[Symbol.toStringTag] = 'Func';
}
