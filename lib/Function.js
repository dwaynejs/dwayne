...
import {
  isFunction, validate, toArray, Symbol, defineProperties, iterate,
...
export function Func(func = () => {}) {
  checkClassInstance(this, Func, 'Func');
  ...
  function proxy() {
    if (++proxy.$$.called < proxy.$$.canBeCalled) {
      ...
      iterate(before, (middleware) => {
        promise = promise.then((args) => middleware.apply(null, toArray(args)));
      });

      promise = promise.then((args) => func.apply(context, toArray(args)));

      iterate(after, (middleware) => {
        promise = promise.then((ret) => middleware(ret));
      });
