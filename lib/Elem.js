...
const typeSwitcher = switcher('strictEquals', (type) => new Elem(nativeDocument.createElement(type)))
  .case('#comment', () => nativeDocument.createComment(''))
  .case('#text', () => nativeDocument.createTextNode(''));
...
export class Elem extends Arr {
  ...
  create(type, ...appliedExpressions) {
    return this.object((elems, elem) => {
      const element = typeSwitcher(type);
  ...
  into(element, end = true) {
    ...
    return this.forEach((elem) => {
      if (!end && element.firstChild) {
        return element.insertBefore(elem, element.firstChild);
      }
