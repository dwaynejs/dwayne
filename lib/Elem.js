...
import {
  constructors, appliedRegExps, htmlElements,
...
export class Elem extends Arr {
   ...
   * or a selector of it.
   * @param {Boolean} [includeAllNodes = false] - If all nodes should be included rather than only elements.
   ...
   */
  child(element, includeAllNodes) {
    if (isInteger(element) && element >= 0) {
      return this.children(includeAllNodes).elem(element);
    }
   ...
   */
  children(includeAllNodes) {
    const childrenProp = includeAllNodes ? 'childNodes' : 'children';

    return new Elem(this.length ? this.$[0][childrenProp] : []);
  }
   ...
   */
  firstChild(selector = null, includeAllNodes) {
    return this.object((elems, elem) => {
   ...
   */
  lastChild(selector = null, includeAllNodes) {
    return this.object((elems, elem) => {
      const { value: found } = new Arr(elem)
        .children(includeAllNodes)
        .reverse()
        .find((elem) => new Elem(elem).is(selector)) || {};
   ...
   */
  next(selector = null, includeAllNodes) {
    const nextSiblingProp = includeAllNodes ? 'nextElementSibling' : 'nextSibling';

    return this.object((elems, elem) => {
      /* eslint no-cond-assign: 0 */
      while (elem = elem[nextSiblingProp]) {
   ...
   */
  prev(selector = null, includeAllNodes) {
    const prevSiblingProp = includeAllNodes ? 'previousElementSibling' : 'previousSibling';

    return this.object((elems, elem) => {
      /* eslint no-cond-assign: 0 */
      while (elem = elem[prevSiblingProp]) {
        if (isNull(selector) || new Elem(elem).is(selector)) {
          return elems.add(elem);
        }
      }
...
dynamicDefineProperties(Elem.prototype, htmlElements, (elem) => function () {
