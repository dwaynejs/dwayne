...
import {
  isArray, isArrayLike, isElement, isFunction, isInteger,
  isNumber, isNull, isObject, isString, isUndefined,
...
export class Elem extends Arr {
  constructor(elem = []) {
    super((() => {
      ...
      return new Arr(toArray(new Super(element).$, true)).object((elems, elem) => {
        if ((isElement(elem) || isWindow(elem) || isHTMLDocument(elem)) && elems.indexOf(elem) === -1) {
          elems.push(elem);
        }
      }, []).$;
    })());
  ...
  blob() {
    return new Promise((resolve, reject) => {
      ...
      elem
        ...
        .then(() => {
          if (elem.isBroken()) {
            throw new Error('The image is broken! (Elem#blob)');
          }
  ...
  changeRule(name, style) {
    this.some((elem) => {
      if (getName(elem) === 'style') {
        const rule = elem.dwayneData.rules.find((rule) => rule.dwayneData && rule.dwayneData.name === name);
  ...
  // TODO: jsdoc
  off() {
    const { removeListeners } = this.$.dwayneData;
  ...
  on(event, selector, listener) {
    ...
    event = new Super(event).object((listeners, listener, event) => {
      iterate(event.split(', '), (event) => {
        listeners[event] = listener;
      });
    });

    this.forEach((elem) => {
      const { removeListeners } = elem.dwayneData || windowsDwayneData.find(({ element }) => element === elem);
      ...
      event.forEach((listener, event) => {
        ...
        listeners.prop(event, (listeners.prop(event) || new Arr()).push(removeListener));
    ...
    return function removeEventListeners(event) {
      if (arguments.length) {
        if (listeners.has(event)) {
          listeners.prop(event).forEach((removeListener) => removeListener());
        }

        return listeners.delete(event);
      }

      listeners.forEach((removeListeners) => {
   ...
   * @method Elem#rect
   * @public
   * @param {Number} [leftCornerX = 0] - See the link.
   * @param {Number} [leftCornerY = 0] - See the link.
   * @param {Number} [width = canvas.width] - See the link.
   * @param {Number} [height = canvas.height] - See the link.
   * @returns {Elem} Returns this.
   * @see https://developer.mozilla.org/en/docs/Web/API/CanvasRenderingContext2D/rect
   * @description Synonym for
   * {@link CanvasRenderingContext2D#rect}[https://developer.mozilla.org/en/docs/Web/API/CanvasRenderingContext2D/rect].
   *
   * @example
   * canvas.rect(10, 10, 30, 30);
   * canvas.rect(10, 10);
   * canvas.rect();
   */
  rect(leftCornerX = 0, leftCornerY = 0, width, height) {
    const ctx = this.ctx();

    if (ctx) {
      const { canvas } = ctx;

      width = isUndefined(width) ? canvas.width : width;
      height = isUndefined(height) ? canvas.height : height;

      ctx.rect(leftCornerX, leftCornerY, width, height);
    }
   ...
   * @public
   * @param {ValidateCallback} validator - Function that validates inputs.
   * @returns {Object.<String, Error|*>|null} Returns either an object
   * with input names keys and errors values or null if no errors found.
   * @description Validates every input element in the set. If an element is a form it validates all input elements inside it.
   *
   * @example
      ...
      .forEach((elem, index) => {
        if (getName(elem) === 'form') {
          return new Elem(elem)
            .find(inputElements)
            .forEach((input, index) => {
              validatorWrap(input.value, input, index);
            });
        }

        validatorWrap(elem.value, elem, index);
      });

    function validatorWrap(value, input, index) {
      try {
        if (input.validity && !input.validity.valid) {
          throw new Error(input.validationMessage);
        }

        validator(value, input, index);
      } catch (err) {
  ...
  if (!{}.hasOwnProperty.call(elem, 'dwayneData') && !isWindow(elem)) {
    ...
    Object.defineProperty(elem, 'dwayneData', {
      value: {
        previousDisplay: '',
        removeListeners: {},
        rules: getName(elem) === 'style' && new Super(elem.cssRules),
