...
export function registerDValue(Mixin) {
  class DValue extends Mixin {
    ...
    constructor(opts) {
      ...
      if (!isFunction(value)) {
        initialScopeValue = parent.$$.evaluate(`{${ _value }}`, (newValue) => {
      ...
      if (
        isNil(initialScopeValue)
        || type === 'checkbox'
        || (name === 'select' && elem.hasAttr('multiple'))
      ) {
    ...
    setProp(value) {
      ...
      if (prop === 'text') {
        ...
        options.forEach((option) => {
          options.selected = value.indexOf(option) !== -1;
        });
      } else {
    ...
    getProp(values) {
      ...
      return prop === 'text'
        ? elem.text()
        : getValueSwitcher(name, [elem.prop(prop), type, node.value, values, elem, options]);
    }
  }
