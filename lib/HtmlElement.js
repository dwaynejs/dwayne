...
export class Elem extends Arr {
   ...
   * elem.apply('@margin-left(2px)'); // also a shorthand for elem.css('marginLeft', '2px 2px');
   ...
   * Fires the event on the first element in the set.
   */
	blur() {
    const elem = this.$[0];
   ...
   */
    ...
    iterate(property, (value, property) => {
      this.forEach((elem) => {
        if (isFunction(value)) {
          value = value(elem.style[property], property, elem);
        }
   ...
   */
	data(key, value) {
    const dataset = getElem(this.$[0]).dataset;
   ...
   */
	defaultValue(value) {
    if (!arguments.length) {
      return getElem(this.$[0]).defaultValue;
    }
   ...
   * @public
   * @param {Boolean|*} [disable = true] - If all the elements in the set should be disabled or enabled.
   * @returns {Elem} Returns this.
   * @description
   */
	disabled(disable = true) {
    return this.toggleAttr('disabled', disable);
	}
