...
export class Elem extends Super {
   ...
   */
    ...
    if (isInteger(element) && element >= 0) {
      return htmlElement(this.$ ? this.$.children[element] : null);
    }
   ...
   * @public
   * @returns {Elems} D-Wrap of the element's children.
   ...
   */
	children() {
		return new Elems(this.$ ? this.$.children : []);
	}
   ...
   */
	clone(deep) {
		return htmlElement(this.$ ? this.$.cloneNode(deep) : null);
	}

  closest(selector) {
    if (element.propertyDescriptor('closest')) {
      return htmlElement(this.$ ? this.$.closest(selector) : null);
    }
   ...
   * @public
   * @param {String|Elem|Element} element - Element to find out if it's within this element or a selector of it.
   ...
   */
    ...
    return !element || !this.$ ? false : this.$.contains(element);
	}
...
dynamicDefineProperties(Elems.prototype, collectionMethods, (prop) => function () {
  return this.forEach((elem) => elem[prop].apply(elem, arguments));
});
