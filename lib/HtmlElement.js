...
export class HtmlElement extends Super {
	constructor(elem = null) {
		...
		if (!isNull(elem) && !this.has('domcData') && this.toStringTag !== 'Window') {
      /**
  ...
  // TODO: add null check in every method
  ...
  addRule(name, selector, style) {
    ...
    sheet.insertRule(new Super(style).word((value, property) => {
      ...
      if (!removeEventListeners.has('listener')) {
        const newListener = (e) => {
          removeEventListeners.forEach(({ selector, listener }) => {
            if (!selector || new HtmlElement(e.target).matches(selector)) {
...
if (supportSymbol) {
  Super.prototype[Symbol.toStringTag] = 'Elem';
}
