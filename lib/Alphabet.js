...
import { isString, validate, defineProperties, Symbol, checkClassInstance } from './helpers';
 ...
 * new Alphabet(['a', 'b', 'c']);
 */
export function Alphabet(alphabet = []) {
  checkClassInstance(this, Alphabet, 'Alphabet');

  const a = new Super(alphabet).object((a, char) => {
    if (!check(char)) {
