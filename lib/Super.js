...
import {
  isArray, isArrayLike, isDate, isElement, isFunction, isNaN, isNull,
  isNullOrUndefined, isObject, isPrimitive, isRegExp, isString, isUndefined,
  validate, toStringTag, iterate, Symbol, defineProperties, checkClassInstance
} from './helpers';
 ...
 * new Super({}); // Super
 */
export function Super(object) {
  checkClassInstance(this, Super, 'Super');
...
defineProperties(Super.prototype, {
   ...
   */
  deepClone() {
    return new Super(deepClone(this.$));
  },
...
function deepAssign(target, object) {
  iterate(object, (value, key) => {
    ...
    if (isRegExp(value) && !hasProperty) {
      target[key] = new RegExp(value.source, value.flags);
    } else if (isPrimitive(target[key]) || !hasProperty) {
...
function deepClone(object) {
  ...
  if (isFunction(object) || isPrimitive(object)) {
    return object;
  }

  return new Super(object).map((value) => deepClone(value)).$;
}
