...
export class Super {
  ...
  static addProperties(property, value) {
   ...
   */
  clone() {
    return new Super(cloneOnce(this.$));
  }
...
function cloneOnce(object) {
  const cl = clone(object);

  if (cl === unique) {
    return new Super(object).map((value) => value).$;
  }
...
function deepAssign(target, object) {
  iterate(object, (value, key) => {
    ...
    if (isRegExp(value) && !hasProperty) {
      ...
      target[key] = isFunction(value) ? value : isArray(value) ? [] : {};
    }
...
function deepClone(object) {
  ...
  if (cl === unique) {
    return new Super(object).map(deepClone).$;
  }
