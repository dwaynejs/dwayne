...
import * as checkTypes from './helpers/checkTypes';
import {
	isArray, isArrayLike, isDate, isFunction, isNaN, isNull, isNullOrUndefined,
  isObject, isPrimitive, isRegExp, isString, isUndefined,
	validate, toStringTag, iterate, assign, supportSymbol, defineProperties
} from './helpers';

/**
 * @typedef {Super|Arr|Func|Elem|Elements|Str|Num|Dat|BlobObject|Alphabet} DWrap
 * @public
...
export class Super {
   ...
   * @method Super#array
   * @public
   * @param {SuperArrayCallback} callback - Called on each iteration.
   * @returns {Arr|Super} Wrap of the array. If Arr class is included then it's an instance of Arr, otherwise - an instance of Super.
   * @description Type of {@link Super#reduce}.
   * The array is passed as an argument into the callback. The output is this array.
   *
   * @example
   * new Super({ a: 1, b: 2 }).array((array, value, key) => {
   *   array.push(key, value);
   * }).$; // ['a', 1, 'b', 2]
   */
	array(callback) {
		validate([callback], ['function'], 'Super#array');

		const a = [];

    iterate(this.$, (value, key, object) => {
      callback(a, value, key, object);
    });
   ...
   * @public
   * @param {?IterationCallback} [callback] - Callback that is passed to {@link Super#sum}.
   ...
   */
	get count() {
		...
		return isObject(object) ? isArrayLike(object) ? object.length : Object.keys(object).length : 0;
	}
   ...
   */
  create(descriptors) {
    return D(Object.create(this.$, descriptors));
  }
   ...
   * @param {Object.<String, Function>} [getter] - If a property parameter is a string this has to be a getter function.
   * @returns {DWrap} Returns this.
   ...
   * @returns {Super} Wrap of the new object.
   * @description Type of {@link Super#reduce}.
   ...
   * @param {Object.<String, Function>} [setter] - If a property parameter is a string this has to be a setter function.
   * @returns {DWrap} Returns this.
  ...
  value(property, value) {
    ...
    if (isObject(object)) {
      iterate(new Super(property).$, (value, property) => {
