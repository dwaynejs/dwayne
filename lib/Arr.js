...
export class Arr extends Super {
   ...
   * @public
   * @param {Array|Arr} array - Array to compare.
   ...
   */
  isContentEqual(array) {
    array = new Arr(array);

    return this.length === array.length && this.every((element) => array.indexOf(element) !== -1);
  }
   ...
   */
  isContentStrictEqual(array) {
    array = new Arr(array);

    return this.length === array.length && this.every((element) => array.indexOfStrict(element) !== -1);
  }
