	...
	describe('it should test D.Array.prototype.[methods]', () => {
		...
		describe('join()', () => {
			it('should return a wrap of joined string', () => {
				const o = [1, 2, 3, 4, 5],
					wrap = new Class(o);
				assert.equal(wrap.join().$, o.join());
				assert.equal(wrap.join('a').$, o.join('a'));
			});
		});
		// TODO: get last
		// TODO: get length
		...
		describe('sort()', () => {
			...
			it('should support "asc" argument and put NaN to the beginning', () => {
				const o = [8, 5, 10, -1, NaN, 6],
					wrap = new Class(o);
				assert.strictEqual(methods.isNaN(wrap.sort('asc').$[0]), true);
			});
			...
			it('should support "desc" argument and put NaN to the end', () => {
				...
				assert.strictEqual(methods.isNaN(array[array.length - 1]), true);
			});
