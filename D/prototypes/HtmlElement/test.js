	...
	describe('it should test D.HtmlElement.prototype.[methods]', () => {
		...
		describe('.apply()', () => {
			it('should set id to the value from the string', () => {
				const elem1 = document.createElement('div'),
					...
					wrap1 = new Class(elem1),
					wrap2 = new Class(elem2),
					wrap3 = new Class(elem3),
					wrap4 = new Class(elem4),
					wrap5 = new Class(elem5);
			...
			it('should put element to the element presented by selector from the string', () => {
				const elem1 = document.createElement('div'),
					...
					parent4 = document.createElement('div'),
					wrap1 = new Class(elem1),
					wrap2 = new Class(elem2),
					wrap3 = new Class(elem3),
					wrap4 = new Class(elem4);
		...
		describe('.[css-property]()', () => {
			it('should check that get methods are working', () => {
				new D.Array(css).forEach((value) => {
					const elem = document.createElement('div'),
						wrap = new Class(elem);
					elem.style[value] = 'initial';
					if (['borderImage', 'pageBreakAfter', 'pageBreakBefore', 'pageBreakInside'].indexOf(value) === -1) {
						assert.strictEqual(wrap[value](), 'initial');
					}
				});
			});
		...
		describe('.css()', () => {
			...
			it('should support (attr, value) syntax', () => {
				const elem = document.createElement('div'),
					wrap = new Class(elem);
				wrap.attr('display', 'inline');
				assert.strictEqual(elem.getAttribute('display'), 'inline');
			});
			it('should support object property syntax', () => {
				const elem = document.createElement('div'),
					wrap = new Class(elem);
		...
		describe('get name', () => {
			it('should return tagName.toLowerCase()', () => {
				const div = document.createElement('div'),
					...
					divWrap = new Class(div),
					inputWrap = new Class(input),
					fieldsetWrap = new Class(fieldset);
